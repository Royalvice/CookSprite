...
export interface JsonSchemaProperty { type?: "integer" | "number" | "string" | "boolean"; title?: string; default?: unknown; minimum?: number; maximum?: number; multipleOf?: number }
export interface ModelOption { id: string; model_id: string; label: string; runtime_id: string; family: string; modes: string[]; params_schema: { type?: string; properties?: Record<string, JsonSchemaProperty>; additionalProperties?: boolean } }
...
export interface WorkerRuntimeIdentity {
  schema: "cooksprite.worker-runtime/v1" | string;
  source_branch: string;
  source_revision: string;
  node_pack_version: string;
...
export interface RuntimeView {
  ...
  runtime_identity?: WorkerRuntimeIdentity | null;
}
...
export interface ComfyProbeCandidate {
  ...
  cooksprite_nodes?: number;
  nodes_installed?: boolean;
  runtime_identity?: Record<string, string> | null;
  error?: string;
}
...
export interface RuntimeDefaultsView {
  ...
  mode_defaults: Record<string, Record<string, RuntimeDefaultBinding>>;
  ...
  recipes: Array<{ id: string; label: string; actions: string[]; modes: string[]; model_id: string; params_schema: ModelOption["params_schema"] }>;
}
...
export const api = {
  ...
  createRuntime: (body: { id?: string; label?: string; base_url: string; location: "local" | "remote"; transport?: string; callback_url?: string; worker_managed?: boolean }) => json<RuntimeRegistration>("/runtimes", { method: "POST", ...jsonBody(body) }),
  ...
  setRuntimeModeDefault: (id: string, actionId: string, mode: string, body: RuntimeDefaultBinding) =>
    json<{ runtime_id: string; action_id: string; mode: string; default: RuntimeDefaultBinding }>(`/runtimes/${encodeURIComponent(id)}/defaults/${encodeURIComponent(actionId)}/modes/${encodeURIComponent(mode)}`, { method: "PUT", ...jsonBody(body) }),
  ...
  doctorRuntime: (id: string) => json<{ runtime_id: string; snapshot: string; tool_count: number; recipe_count: number; system: Record<string, unknown>; device?: Record<string, unknown>; models: Record<string, number>; recipes: RuntimeRecipe[]; worker_managed: boolean; runtime_identity?: WorkerRuntimeIdentity | null }>(`/runtimes/${encodeURIComponent(id)}/doctor`, { method: "POST" }),
  probeComfy: (baseUrl: string) => json<ComfyProbeView>("/comfyui/probe", { method: "POST", ...jsonBody({ base_url: baseUrl }) }),
};
